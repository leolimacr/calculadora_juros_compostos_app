...
const MobileMenu: React.FC<MobileMenuProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-[200] flex justify-end font-sans">
      ...
      <div className="relative w-[80%] max-w-[320px] bg-white h-full shadow-2xl border-l border-slate-200 animate-in slide-in-from-right duration-300 flex flex-col">
        
        {/* HEADER: SEJA BEM VINDO */}
        <div className="p-6 pt-10 border-b border-slate-200">
          <div className="flex justify-between items-start mb-4">
            <div>
              <p className="text-[10px] font-black text-sky-600 uppercase tracking-[0.2em] mb-1">Área do Investidor</p>
              <h2 className="text-slate-900 text-xl font-black tracking-tight">SEJA BEM VINDO,<br/> <span className="text-sky-600">{firstName}!</span></h2>
            </div>
            <button onClick={onClose} className="p-2 bg-surface-secondary rounded-lg text-slate-500">
              <X size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 pt-6">
          ...
          <button 
            ...
            className="w-full group relative overflow-hidden rounded-2xl bg-white border border-amber-300 shadow-sm p-4 transition-all active:scale-95"
          >
            ...
            <div className="flex items-center gap-4 relative z-10">
              ...
              <div className="flex-1 text-left">
                <span className="block text-[10px] font-black uppercase text-amber-600 tracking-widest">Gerenciador financeiro</span>
                <span className="block text-[15px] font-bold text-slate-900 uppercase leading-tight">Controla</span>
              </div>
              <ChevronRight size={18} className="text-slate-400 group-hover:text-amber-600" />
            </div>
          </button>
          ...
          <button 
            ...
            className="w-full group relative overflow-hidden rounded-2xl bg-white border border-sky-300 shadow-sm p-4 transition-all active:scale-95"
          >
            ...
            <div className="flex items-center gap-4 relative z-10">
              ...
              <div className="flex-1 text-left">
                <span className="block text-[10px] font-black uppercase text-sky-600 tracking-widest">Inteligência Artificial</span>
                <span className="block text-[15px] font-bold text-slate-900 uppercase leading-tight">Nexus IA</span>
              </div>
          ...
          <button 
            ...
            className="w-full group flex items-center gap-4 p-4 rounded-2xl bg-surface-secondary border border-slate-200 hover:bg-slate-100 transition-all active:scale-95"
          >
          ...
          <div className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-200">
            <div className={`p-2 rounded-lg ${isPrivacyMode ? 'bg-emerald-500/20 text-emerald-500' : 'bg-slate-100 text-slate-500'}`}>
              {isPrivacyMode ? <EyeOff size={20}/> : <Eye size={20}/>}
            </div>
            <span className="flex-1 text-left font-bold text-slate-500 uppercase text-[12px]">Modo Privado</span>
            <button onClick={onTogglePrivacy} className={`w-10 h-5 rounded-full relative transition-colors ${isPrivacyMode ? 'bg-emerald-600' : 'bg-slate-300'}`}>
        ...
        <div className="p-6 border-t border-slate-200 bg-slate-50">
          ...
          <div className="mt-4 flex justify-center items-center gap-2 text-[10px] text-slate-400 font-bold uppercase tracking-widest">
