        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-surface-elevated">
            {transactions.length === 0 ? (
              ...
              transactions.map((t) => {
                const isFutureLaunch = Boolean((t as any).isFutureLaunch || t.isVirtual);
                const rowClass = isFutureLaunch
                  ? 'bg-amber-50/80 hover:bg-amber-100/70 transition-colors group align-middle border-l-4 border-l-amber-400'
                  : 'hover:bg-surface-secondary/50 transition-colors group align-middle';
                const categoryClass = isFutureLaunch
                  ? 'inline-flex items-center px-2.5 py-1 bg-amber-100 border border-amber-200 rounded-full text-xxs font-black text-amber-800 uppercase tracking-wider'
                ...
                return (
                  <tr key={t.id} className={rowClass}>
                    <td className="px-4 py-3 text-xxs text-text-secondary font-medium whitespace-nowrap">
                      {isFutureLaunch ? 'Lançamento futuro' : new Date(t.date.replace(/-/g, '/')).toLocaleDateString('pt-BR')}
                      {isFutureLaunch && (t as any).dueDate && (
                    ...
                    <td className="px-4 py-3 text-sm text-text-primary font-bold leading-tight">
                      <div className="flex flex-col gap-1">
                        {t.description}
                        {isFutureLaunch && (
                          <span className="inline-flex w-fit items-center px-1.5 py-0.5 bg-amber-100 border border-amber-200 rounded-md text-[9px] font-black text-amber-800 uppercase tracking-wider">
                            Lançamento futuro
                          </span>
                        )}
                    ...
                    <td className={`px-4 py-3 text-right font-black text-sm whitespace-nowrap ${isFutureLaunch ? 'text-amber-700' : t.type === 'income' ? 'text-brand-primary' : 'text-status-danger'}`}>
