...
export const DebtManager: React.FC<DebtManagerProps> = ({ userId, userMeta, onNavigate }) => {
  const { isPro } = useSubscriptionAccess(userId);
          ...
          <div className="lg:col-span-2 bg-slate-950 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <div className="space-y-3">
                ...
                <p className="text-[11px] text-slate-400 font-medium italic">O Finanças Pro Invest está monitorando {totalRemainingParcelas} parcelas em aberto.</p>
              </div>
            </div>
          </div>
          {criticalDebt && (
          ...
          {savedPlans.map(plan => (
            <div key={plan.id} className="bg-white border border-slate-200 rounded-[2rem] p-6 shadow-sm flex flex-col justify-between group hover:border-sky-300 transition-all">
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Gerado em {plan.createdAt?.toDate ? plan.createdAt.toDate().toLocaleDateString('pt-BR') : 'Recente'}</p>
              </div>
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Dívida</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Restantes</label>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl">
             <button onClick={() => setViewMode('grid')} className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-white shadow-sm' : 'text-slate-400'}`}><LayoutGrid size={16} /></button>
             <button onClick={() => setViewMode('list')} className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-white shadow-sm' : 'text-slate-400'}`}><List size={16} /></button>
          </div>
        </div>
        ...
        {debts.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {debts.map(debt => (
              <div key={debt.id} className="bg-white border border-slate-200 rounded-[2.5rem] p-7 shadow-sm group relative overflow-hidden flex flex-col h-full">
                <div className={`absolute top-0 left-0 w-full h-2 ${STRIPE_COLORS[debt.tipo] || 'bg-slate-400'}`} />
                <div className="flex justify-between items-start mb-6 mt-2">
                  <div>
                    <span className="text-[9px] font-black text-slate-400 uppercase">{debt.tipo}</span>
                    <h4 className="text-lg font-black text-slate-900">{debt.nome}</h4>
                  </div>
                  <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                    <button onClick={() => setSelectedDebtForProjection(debt)} className="p-2.5 rounded-xl bg-slate-50 text-slate-400 hover:text-emerald-600 transition-all border border-slate-100"><TrendingUp size={16} /></button>
                    <button onClick={() => handleEdit(debt)} className="p-2.5 rounded-xl bg-slate-50 text-slate-400 hover:text-sky-600 transition-all border border-slate-100"><Pencil size={16} /></button>
                    <button onClick={() => debt.id && handleDelete(debt.id)} className="p-2.5 rounded-xl bg-slate-50 text-slate-400 hover:text-rose-600 transition-all border border-slate-100"><Trash2 size={16} /></button>
                  </div>
                </div>
          ...
          <div className="bg-white border rounded-[2.5rem] overflow-hidden shadow-sm">
             <table className="w-full text-left">
               <tbody className="divide-y divide-slate-50">
                 {debts.map(debt => (
                   <tr key={debt.id} className="hover:bg-slate-50/50 group">
                     <td className="px-8 py-5 font-black text-slate-900">{debt.nome}</td>
                     <td className="px-8 py-5 text-right font-black text-rose-600">{formatCurrency(debt.saldoDevedor)}</td>
                     <td className="px-8 py-5">
                       <button onClick={() => handleEdit(debt)} className="p-2 text-slate-400 hover:text-sky-600"><Pencil size={14} /></button>
                       <button onClick={() => debt.id && handleDelete(debt.id)} className="p-2 text-slate-400 hover:text-rose-600"><Trash2 size={14} /></button>
                     </td>
                   </tr>
                 ))}
               </tbody>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-[3rem] p-10 max-w-md w-full shadow-2xl animate-in zoom-in-95">
             ...
             <div className="flex flex-col gap-3">
               ...
               <button onClick={() => setShowConfirmModal(false)} className="w-full py-5 bg-slate-50 text-slate-400 rounded-2xl font-black uppercase hover:bg-slate-100 transition-all">Cancelar</button>
             </div>
          </div>
        </div>
      )}
