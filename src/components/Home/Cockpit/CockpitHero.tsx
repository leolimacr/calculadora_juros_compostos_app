...
const CockpitHero: React.FC<CockpitHeroProps> = ({
  ...
  // Fonte única: shortfalls vêm do snapshot soberano (não recalculados aqui).
  const colchaoShortfall = sovereign.colchaoShortfall;
  const reserveShortfall = sovereign.reserveShortfall;
  const totalShortfall = sovereign.protectionShortfall;

  const heroValueDisplay = isPrivacyMode
    ? '••••••'
    : formatCurrency(sovereign.heroValue);

  return (
    <div className="relative overflow-hidden rounded-panel bg-white border border-border-strong p-6 md:p-8 shadow-floating group">
        ...
        <div className="space-y-2 flex-1">
          {urgentBills.length > 0 && (
            ...
            >
              ...
              <p className="text-lg font-black tracking-tight leading-tight text-slate-950">
                {urgentBills.length} {urgentBills.length === 1 ? 'conta vence' : 'contas vencem'}{' '}
                <span className="text-emerald-600">hoje ou amanhã.</span>
              </p>
            </div>
          )}
              <p className="text-slate-500 text-[10px] font-black uppercase tracking-[0.25em] mb-3">{greeting}, {userMeta?.nickname || 'Investidor'}</p>
              <div className="flex flex-wrap items-center gap-3">
                <span className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-xl border text-[10px] font-black uppercase tracking-widest shadow-sm ${
                  isPremium ? 'bg-white text-emerald-700 border-emerald-200 shadow-emerald-500/10' :
                  isPro ? 'bg-white text-sky-700 border-sky-200 shadow-sky-500/10' :
                ...
                {userMeta?.persona?.archetype && (
                  <span className="inline-flex items-center gap-2 px-4 py-1.5 rounded-xl border border-slate-200 bg-slate-50 text-[10px] font-black uppercase tracking-widest text-slate-500">
                    Estratégia: {(userMeta.persona.archetype as string) === 'dividas' ? 'Sair do vermelho' : (userMeta.persona.archetype as string) === 'patrimonio' ? 'Crescimento' : 'Controle'}
                  </span>
                )}
              </div>
              <div className="flex flex-col md:flex-row md:items-end gap-8 mt-6">
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    ...
                    {sovereign.commandMode && (
                      <button
                        ...
                        className="flex items-center gap-1 text-[10px] font-bold text-slate-500 hover:text-slate-700 transition-colors py-1 min-h-[44px]"
                    ...
                    {expandedSection === 'hero' && sovereign.commandMode && (
                      <div className="rounded-2xl bg-surface-subtle border border-slate-200 p-4 space-y-2 animate-in fade-in slide-in-from-top-1 duration-200">
                        ...
                        <div className="space-y-1.5 text-[12px] font-medium text-slate-700">
                          <div className="flex justify-between">
                            ...
                            <span className="font-black">{formatCurrency(sovereign.accumulatedBalance - exclusionAmount)}</span>
                          </div>
                      ...
                      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 pt-1">
                        ...
                        <div className="relative">
                          <button
                            ...
                            className="flex items-center gap-1 text-[9px] font-bold text-slate-500 uppercase tracking-widest hover:text-slate-700 transition-colors py-1 min-h-[36px]"
                          >
                            <span>Acumulado: {isPrivacyMode ? '•••' : formatCurrency(sovereign.accumulatedBalance - exclusionAmount)}</span>
                          ...
                          {expandedInline === 'saldoAcumulado' && (
                            <div className="absolute left-0 top-full mt-1 z-20 w-72 rounded-2xl bg-white border border-slate-200 shadow-lg p-4 space-y-1.5 animate-in fade-in slide-in-from-top-1 duration-200">
                              ...
                              <div className="border-t border-slate-100 pt-1 flex justify-between text-[11px] font-black">
                                <span>= Saldo acumulado</span>
                                <span>{formatCurrency(sovereign.accumulatedBalance - exclusionAmount)}</span>
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="relative">
                          <button
                            ...
                            className="flex items-center gap-1 text-[9px] font-bold text-amber-700 uppercase tracking-widest hover:text-amber-800 transition-colors py-1 min-h-[36px]"
                          >
                          ...
                          {expandedInline === 'cartao' && (
                            <div className="absolute left-0 top-full mt-1 z-20 w-72 rounded-2xl bg-white border border-slate-200 shadow-lg p-4 space-y-2 animate-in fade-in slide-in-from-top-1 duration-200">
                              ...
                              <div className="flex justify-between text-[11px] font-black">
                                ...
                                <span className="text-amber-700">{formatCurrency(sovereign.virtualImpact)}</span>
                              </div>
                              {onNavigate && (
                                <button
                                  type="button"
                          ...
                          {expandedInline === 'contas' && (
                            <div className="absolute left-0 top-full mt-1 z-20 w-72 rounded-2xl bg-white border border-slate-200 shadow-lg p-4 space-y-1.5 animate-in fade-in slide-in-from-top-1 duration-200">
                              ...
                              <div className="flex justify-between text-[11px] font-black">
                                ...
                                <span className="text-amber-700">{formatCurrency(sovereign.totalPendingBills)}</span>
                              </div>
                            </div>
                          )}
                        </div>
                          ...
                          <>
                            ...
                            <div className="relative">
                              <button
                                ...
                                className="flex items-center gap-1 text-[9px] font-bold text-sky-700 uppercase tracking-widest hover:text-sky-800 transition-colors py-1 min-h-[36px]"
                              >
                  ...
                  {sovereign.commandMode && sovereign.leewayDays > 0 && !isPrivacyMode && (
                    <p className="text-[11px] font-black text-emerald-700 uppercase tracking-widest flex items-center gap-2 pt-1">
                  ...
                  {!isPrivacyMode && sovereign.commandMode && (
                    <div className="flex flex-wrap gap-x-4 gap-y-2 pt-2">
                      {(sovereign.totalCreditUsed ?? 0) > 0 && (
                        <div className="flex items-center gap-1.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-amber-600" />
                          <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">
                            Crédito Comprometido: {formatCurrency(sovereign.totalCreditUsed ?? 0)}
                          </span>
                        </div>
                      )}
                      {(sovereign.totalDebtBalance ?? 0) > 0 && (
                        <div className="flex items-center gap-1.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-rose-600" />
                          <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest">
                            Peso das Dívidas: {formatCurrency(sovereign.totalDebtBalance ?? 0)}
                          </span>
                        </div>
                      )}
                    </div>
                ...
                <div className="flex flex-col gap-2 pb-2">
                  {!sovereign.commandMode && sovereign.totalPendingBills > 0 && !isPrivacyMode && (
                    <div className="flex items-center gap-3 px-5 py-2 bg-surface-subtle border border-slate-200 rounded-2xl shadow-inner">
                      <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                        O que sobra:{' '}
                        <span className={`text-sm ${sovereign.projectedBalance >= 0 ? 'text-emerald-700' : 'text-status-danger'}`}>
